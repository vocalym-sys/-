(function () {
  ...
  // 이 근사치는 원래 소령도 부근 점(C, 36.93N)에서 뚝 끊겨 있었음 —
  // 사용자가 국립해양조사원 참고 지도(점선이 소령도 이북 위도
  // 37.17도까지 매끄럽게 이어짐)를 보여주며 이 부분이 빠졌다고 지적함.
  // 마침 TERRITORIAL_LIMIT_OFFICIAL(공식 shapefile)의 본토 선 시작
  // 구간이 정확히 이 원호 부분(37.17도에서 36.90도까지, C점과 불과
  // 3.4km 차이 나는 지점까지 내려옴)이라 그 실제 좌표를 그대로 이어
  // 붙임 — 나머지 구간(NLL 방향)은 여전히 안 그림, 이 자연스러운
  // 원호 연장만 보정.
